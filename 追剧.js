var rule = {
  ...
  lazy:$js.toString(()=>{
     ...
     let json_data = JSON.parse(request('https://zjuys.com/api/mw-movie/anonymous/v1/video/episode/url?id='+pid+'&nid='+nid,{headers: {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36',
  ...
  一级: $js.toString(()=>{
        ...
        list.forEach((it)=>{
            url = 'https://zjuys.com/detail/'+it.vodId
            d.push({
              title: it.vodName,
              desc:it.vodRemarks,
              img:it.vodPic,
  ...
  搜索: $js.toString(()=>{
    ...
    't': t
      ...
      let d = []
      data.forEach(it=>{
        let reurl = 'https://zjuys.com/detail/'+it.vodId
        d.push({
          title: it.vodName,
          desc:it.vodVersion,
          img:it.vodPic,
